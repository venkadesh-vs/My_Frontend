...
async function loadLedger(customerId, userId) {
  ...
  const summaryAmount = document.querySelector(
    ".account-summary .outstanding-amount",
  );
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/ledger/${customerId}?user_id=${userId}`,
    );
    ...
    if (summaryAmount)
      summaryAmount.textContent = formatCurrency(data.outstanding_balance);
